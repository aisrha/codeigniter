<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>css/form.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>css/heading.css">
<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>css/button.css">

<br>
<div class="container">
	<br>
	<button id="buttonForm"><a href="<?php echo base_url(); ?>client/Client_display"><i class='fas fa-backward' id="icon"></i>&nbsp</a></button>
</div>

<h2>ADD CLIENT</h2><br>
<div class="box" align="center">
	<form method="post">
		<table>
			<tr>
				<td class="rowTitle">Name</td><br>
				<td><input type="text" name="name" size="30" required/></td>
			</tr>
			<tr><td><br></td></tr>
			<tr>
				<td><button id="button"><a href="http://localhost/codeigniter/client/Client_displayTableClient">CLIENT LIST</a></button></td>
				<td><input id="button" type="submit" name="Client_addButton" value="SUBMIT" required/></td>
			</tr>
		</table>
	</form>

</div><br><br>